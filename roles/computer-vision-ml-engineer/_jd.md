...
Unlike our AI Product Engineer role, this role **IS about training real computer-vision models**. It is about turning raw video from tens of thousands of vehicles into **accurate, real-time detection that runs cost-effectively at the edge** — and shipping it to production.
...
- Not a classical-ML / tabular-only role — this is **vision-first**
...
1. **Research-only, never shipped** — papers / Kaggle but no production model
2. **Ignores cost & latency** — "just use the biggest model on cloud GPUs"
3. **Can only wrap APIs** — can't train, fine-tune, or optimize a model
4. **Accuracy tunnel-vision** — no thought for edge, cost, or deployment
...
**Only care about:** can they train a model that works in the field AND runs cheaply at scale?
